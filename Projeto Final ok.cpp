...
int indice2 = 0; //verifica se ainda existe cadastros depois da exclusão
int i=0;
...
void consultar();
void voltarAoMenu();
void msgOpcaoInvalida(); //Mensagem padrão de "opção inválida", para evitar a repetição de código
void msgAlterado(); //Mensagem padrão de "alteração realizada com sucesso", evitando repetição de código 
void msgNaoHaClientes(); //Mensagem padrão de que não há clientes cadastrados, para evitar a repetição de código
void msgCadNaoEncontrado(); //Mensagem padrão de "cadastro não enontrado", para evitar a repetição de código
struct cliente c[MAX];

main () {
	char op; //Opção em char pois se alguém comentar um caracter que não seja número, o sistema reconhecerá
...
void alterar(){
	int cod; // Recebe o código do cliente que terá os dados alterados
	int flag;
	int op2; //Recebe o dado(nome,telefone...) que a pessoa quer alterar  
	...
	if (indice==0){
		...
		printf ("Deseja realizar a pesquisa por código(1) ou por nome(2)? (0 para voltar ao menu)\n");
		scanf ("%i", &op3);
		if ((op3>2) || (op3<0)){
			msgOpcaoInvalida();
		}
...
void excluir(){ //Somente aceita códigos
	...
	if (indice==0){
		...
		printf ("\n");
		printf ("Digite o código do cliente que será excluído: (0 para voltar ao menu) ");
		scanf ("%i", &codd);
		if (codd==0){
			voltarAoMenu();
		}
...
void consultar (){
	...
	if (indice==0){
		...
		system("cls");
		printf("Deseja realizar a consulta por código(1) ou nome(2)? (0 para voltar ao menu)");
		scanf ("%i", &cons);
		system("cls");
		if ((cons<0) || (cons>2)){
			msgOpcaoInvalida();
